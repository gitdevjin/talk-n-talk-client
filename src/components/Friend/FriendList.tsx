...
export default function FriendList() {
  ...
  const createDm = async (friendId: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_TNT_SERVER_URL}/chats/dms/${friendId}`, {
        method: "POST",
        credentials: "include",
      });
  ...
  return (
    <div className="flex flex-col gap-1 p-3">
      ...
      {friends.map((f) => (
        <div
          key={f.id}
          className="group flex items-center justify-between px-3 py-2 rounded-lg hover:bg-[var(--color-darkgrey-1)] transition"
        >
